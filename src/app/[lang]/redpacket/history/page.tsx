      ...
      <div className="p-6 rounded-xl bg-[#1a1a1a]">
        <div className="flex justify-between items-center mb-4">
          <span className="text-white/80">
            {t("redpacket.history.title")} {formattedId}
          </span>
        ...
        <div className="flex justify-between items-center text-sm">
          <span className="text-white/80">
            {info.isRefunded ? (
              ...
              `${info.claimed} / ${info.totalCount} ${t(
                "redpacket.history.count"
              )}`
            )}
          </span>
          <span className="text-[#FFD700]">
            {info.totalAmount} {t("redpacket.unit")}
          </span>
        </div>
      </div>
    </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-4">
            {packets.length > 0 ? (
              packets.map((id) => <RedPacketCard key={id.toString()} id={id} />)
            ) : (
